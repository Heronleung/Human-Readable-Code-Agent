"""DeepSeek rule-delta interpretation transport (P4.8).

The distinct, backend-owned module that performs the one permitted hosted
DeepSeek request for *rule-delta interpretation* — separate from, and never a
widening of, the P4.2b advisory transport. It implements the P2.5
:class:`hrca.provider.Provider` interface and is the only place that reads the
credential, opens a socket, or sends the selected requirement text. The desktop
client and the frozen scan/serve/readiness paths never import this module
(enforced by :mod:`tests.test_architecture` and by lazy import at the boundary).

Hard controls, all code-owned (never user-configurable):

* **fixed origin** — :data:`hrca.deepseek.API_ORIGIN` plus the fixed chat
  completions path; no endpoint, proxy, redirect target or header is accepted
  from anywhere, and redirects are never followed (a 3xx status is a bounded
  failure, not a follow);
* **allowlisted model** — :data:`hrca.rule_delta_interpret.MODEL_ID` only;
...
* **thinking disabled** — ``{"thinking": {"type": "disabled"}}`` is always sent;
...
* **bounded request** — 12 KiB serialized body and 4,096 input tokens checked
  before any network access;
* **bounded output** — ``max_tokens`` (1,024) caps the provider output, and the
  returned payload is validated against the bounded interpretation contract
  (:func:`hrca.rule_delta_interpret.validate_provider_output`).

Failures are raised as a bounded :class:`TransportError` whose ``code`` is one
of the normalized interpretation states, so the boundary can map every
transport outcome to a bounded result without leaking a credential, an endpoint
detail, a raw response, or an underlying exception.
"""
...
from . import deepseek, provider, rule_delta_interpret
...
# HTTP status -> bounded code, for the well-known DeepSeek statuses. A redirect
# (3xx) is deliberately absent and falls through to ``provider_failure`` — the
...
def _code_for_status(status: int) -> str:
    """Map an HTTP status to a bounded transport code (redirects never follow)."""
...
def _default_http_post(
    ...
    transport failure; the caller maps those to bounded codes. ``http.client``
    does not follow redirects, so a 3xx is returned as a status and mapped to a
    bounded failure — the transport never connects to an arbitrary origin.
    """
...
def _parse_json_content(content: str) -> Optional[Dict[str, Any]]:
    """Parse a provider JSON object, rejecting prose-wrapped JSON.

    Unlike the advisory transport this does *not* tolerate a markdown code
    fence: prose-wrapped JSON is an invalid output for the strict, data-only
    interpretation contract and must fail closed.
    """
    text = content.strip()
    try:
...
class DeltaInterpretProvider(provider.Provider):
    """The bounded DeepSeek rule-delta interpretation transport.
    ...
    def __init__(
        self,
        *,
        model: str = rule_delta_interpret.MODEL_ID,
        credential_getter: Optional[Callable[[], Optional[str]]] = None,
        http_post: Optional[Callable[..., Tuple[int, bytes]]] = None,
        timeout: float = rule_delta_interpret.TIMEOUT_SECONDS,
        max_output_tokens: int = rule_delta_interpret.MAX_OUTPUT_TOKENS,
        max_request_bytes: int = rule_delta_interpret.MAX_REQUEST_BYTES,
        max_input_tokens: int = rule_delta_interpret.MAX_INPUT_TOKENS,
    ) -> None:
        if model != rule_delta_interpret.MODEL_ID:
    ...
    def capabilities(self) -> provider.ProviderCapabilities:
        return provider.ProviderCapabilities(
            ...
            cancellation=False,
            tool_calling=False,
            context_limit=rule_delta_interpret.MAX_INPUT_TOKENS,
    ...
    def generate(self, request: provider.ProviderRequest) -> provider.ProviderResult:
        ...
        if len(payload_bytes) > self._max_request_bytes:
            raise TransportError("over_limit")
        total_text = request.task + "\n\n" + "\n\n".join(request.context)
        if rule_delta_interpret.estimate_tokens(total_text) > self._max_input_tokens:
            raise TransportError("over_limit")
        ...
        if payload is None:
            raise TransportError("invalid_output")
        reason = rule_delta_interpret.validate_provider_output(payload)
        if reason is not None:
            raise TransportError("invalid_output")

        normalized = rule_delta_interpret.normalize_output(payload)
        # ``content`` is deliberately the *sanitized* interpretation object,
        # never the raw model output, so no requirement text or hidden reasoning
        # can be retained or logged downstream.
        content = rule_delta_interpret.dumps(normalized)
        usage = None
        try:
            data = json.loads(body.decode("utf-8"))
    ...
    def _build_body(self, request: provider.ProviderRequest) -> Dict[str, Any]:
        """Build the OpenAI-compatible chat-completions body with thinking off."""
        user_text = "\n\n".join(request.context)
        return {
            "model": self._model,
            "messages": [
