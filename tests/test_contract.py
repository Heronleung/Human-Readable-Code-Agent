...
class ContractVersionTests(unittest.TestCase):
    def test_contract_version_is_pinned(self):
        # 3.7.0 adds the bounded Memory query protocol (M4.4/v1): cross-run
        # Search/Timeline and Resume. The increment is additive — both new
        # actions are read-only, no storage or index is added, and no prior
        # action's semantics or version rule changes.
        self.assertEqual(CONTRACT_VERSION, "3.7.0")
